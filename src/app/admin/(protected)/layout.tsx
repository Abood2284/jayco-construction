...
import { requireAdminUser, signOutAdmin } from "@/lib/admin/auth"
...
const adminNavItems = [
	...
	{ label: "Inquiries", href: "/admin/contact-submissions" },
			...
			<div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:grid lg:grid-cols-[240px_minmax(0,1fr)] lg:px-8">
				<aside className="rounded-lg border border-slate-200 bg-white">
					...
					<form action={logoutAction} className="border-t border-slate-200 p-2">
						<button
							type="submit"
							className="w-full rounded-md px-3 py-2 text-left text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-950"
						>
							Log out
						</button>
					</form>

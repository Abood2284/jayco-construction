...
function CareersDisciplinedSection({
	variant = "standalone",
	className,
}: CareersDisciplinedSectionProps) {
	if (variant === "card")
		return (
			<div
				className={`relative h-full overflow-hidden rounded-3xl bg-linear-to-r from-[#b71c1c] to-[#ef4444] p-6 shadow-[0_35px_90px_rgba(183,28,28,0.25)] ${className ?? ""}`}
			>
				...
				<div className="relative">
					...
					<div className="mt-6">
						<p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-white/90">Careers</p>

						<h2 className="text-[clamp(1.35rem,2vw,1.75rem)] font-bold leading-tight tracking-tight text-white">
							Join the people behind disciplined manufacturing.
						</h2>

						<p className="mt-4 max-w-[46ch] text-sm font-medium leading-relaxed text-white/85">
						...
						<div className="mt-6">
							<Link
								href="/careers"
								className="inline-flex min-h-[44px] items-center gap-2 rounded-md bg-white px-6 py-3 text-xs font-semibold uppercase tracking-wide text-[#b71c1c] shadow-sm transition-colors hover:bg-[#ffe5e5]"
							>
								View Careers
	...
	return (
		<section className={`bg-slate-50 py-16 lg:py-20 ${className ?? ""}`}>
			<div className="mx-auto max-w-6xl px-4 lg:px-6">
				<div className="relative overflow-hidden rounded-3xl bg-linear-to-r from-[#b71c1c] to-[#ef4444] p-6 shadow-[0_35px_90px_rgba(183,28,28,0.25)] lg:p-10">
					<div className="pointer-events-none absolute inset-0 bg-[radial-gradient(600px_circle_at_15%_20%,rgba(255,255,255,0.14),transparent_45%),radial-gradient(500px_circle_at_85%_60%,rgba(0,0,0,0.18),transparent_55%)]" />
...
interface CareersDisciplinedSectionProps {
